    ...
    <div class="bg-secondary px-10 md:px-16 xl:px-20 py-20 text-primary relative">
      ...
      <div class="pt-2">
        ...
        <div class="grid grid-cols-3 gap-x-6">
          ...
          <div v-for="(item, index) in count_array" :key="index">
            <div class="py-3 font-jost text-2xl w-full bg-secondary">Other Image {{ item }}</div>
            <input
              type="file"
              @change="upload_avatar2"
              @click="index_value = index"
              accept="image/png, image/gif, image/jpeg"
              :ref="'image' + index"
            />
            <input
              type="text"
              v-model="form.image_name[index]"
              placeholder="Image Title"
              class="font-jost text-2xl border-0 focus:ring-0 w-full bg-secondary px-0 mt-2 placeholder-current"
            />
            <div class="border-b border-secondary-4"></div>
            <img :src="get_avatar2(index)" class="w-full rounded-md mt-4 md:mt-6" />
          </div>
        </div>
        <!-- <div class="mt-4 md:mt-6 px-2 md:px-0 grid grid-cols-3 gap-x-6">
